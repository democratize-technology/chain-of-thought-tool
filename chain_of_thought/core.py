...
class ChainOfThought:
    ...
    def _validate_reasoning_stage_param(self, reasoning_stage: str) -> str:
        """
    ...
    def _validate_step_parameters(self, step_number: int, total_steps: int) -> tuple:
        """
    ...
    def _validate_confidence_param(self, confidence: float) -> float:
        """
    ...
    def _validate_integer_list_param(self, int_list: Optional[List[int]], param_name: str) -> Optional[List[int]]:
        """
        ...
        Raises:
            ValueError: If list is invalid
        """
        return self._validate_list_param(
            int_list, param_name, int,
            max_items=50, item_range=(-10000, 10000000)
        )

    def _validate_string_list_param(self, str_list: Optional[List[str]], param_name: str) -> Optional[List[str]]:
        """
    ...
    def _validate_input(
        ...
        thought_cleaned = self._validate_thought_param(thought)
        reasoning_stage_cleaned = self._validate_reasoning_stage_param(reasoning_stage)
        validated_step_number, validated_total_steps = self._validate_step_parameters(step_number, total_steps)
        validated_confidence = self._validate_confidence_param(confidence)
        validated_next_step_needed = self._validate_boolean_param(next_step_needed, "next_step_needed")

        # Validate list parameters
        dependencies_cleaned = self._validate_integer_list_param(dependencies, "dependencies")
        contradicts_cleaned = self._validate_integer_list_param(contradicts, "contradicts")
        evidence_cleaned = self._validate_string_list_param(evidence, "evidence")
        assumptions_cleaned = self._validate_string_list_param(assumptions, "assumptions")
